...
VISUAL_MAPPING: tuple[tuple[str, ...], ...] = (
    ...
    ("item_stone_chisel",),
    ("item_wicker_basket",),
    ("item_straw_roof", "feature_straw_roof"),
    ("item_copper_nails",),
    ("item_iron_nails",),
    ("item_copper_saw",),
    ("item_iron_saw",),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import the expanded survival item atlas.")
    parser.add_argument("atlas", type=Path, help="Path to the transparent ChatGPT atlas PNG.")
    ...
    parser.add_argument("--strict", action="store_true", help="Require exactly 36 visible components.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    components = _select_components(_alpha_components(atlas), len(VISUAL_MAPPING))
    if args.strict and len(components) != len(VISUAL_MAPPING):
    ...
    for component, sprite_ids in zip(components, VISUAL_MAPPING, strict=False):
    ...
    if args.preview:
        ...
        pygame.image.save(_make_contact_sheet(previews), str(GENERATED_DIR / "expanded_survival_import_contact.png"))
        pygame.image.save(_draw_detection_preview(atlas, components), str(GENERATED_DIR / "expanded_survival_detected.png"))

    pygame.quit()
    print(f"Detected components: {len(components)} / {len(VISUAL_MAPPING)}")
    print(f"Sprites written: {len(written)}")
    for sprite_id in written:
        print(f"- {sprite_id}")
    if len(components) < len(VISUAL_MAPPING):
        missing = VISUAL_MAPPING[len(components) :]
        print("Atlas did not include enough visible components for:")
        for sprite_ids in missing:
            print(f"- {', '.join(sprite_ids)}")
    if args.preview:
        print(f"Preview: {GENERATED_DIR / 'expanded_survival_import_contact.png'}")
    return 0
...
def _select_components(components: list[Component], limit: int) -> list[Component]:
    useful = [component for component in components if component.area >= 420 and component.width >= 18 and component.height >= 18]
    useful = sorted(useful, key=lambda component: component.area, reverse=True)[:limit]
    ...
    for component in sorted(useful, key=lambda component: component.center_y):
        for row in rows:
            ...
            if abs(component.center_y - row_center) < 80:
