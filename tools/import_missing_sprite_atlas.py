...
# The prompt asked for a 6x6 atlas, but this generated image left a few requested
# cells blank and shifted several later sprites. This list follows the actual
# visible components from left to right, top to bottom.
VISUAL_MAPPING: tuple[tuple[str, ...], ...] = (
    ("feature_salmon_school",),
    ("feature_eel",),
    ("feature_rabbit",),
    ("feature_deer",),
    ("feature_fox",),
    ...
    ("item_iron_ingot",),
    ("item_gold_ingot",),
    ("feature_wooden_crate",),
    ("item_plank",),
    ("skip_copper_ingot_alt",),
    ("skip_iron_ingot_alt",),
    ("skip_gold_ingot_alt",),
    ("item_wild_seed",),
    ("skip_plank_alt",),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import the 2026-06-07 generated placeholder-replacement atlas.")
    parser.add_argument("atlas", type=Path, help="Path to the generated 6x6 transparent PNG atlas.")
    parser.add_argument("--out", type=Path, default=SPRITE_DIR, help="Output sprite directory.")
    parser.add_argument("--preview", action="store_true", help="Write a contact sheet preview to assets/generated.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for component, sprite_ids in zip(components, VISUAL_MAPPING, strict=True):
        crop = atlas.subsurface(_padded_rect(component.rect(), atlas.get_size(), 14)).copy()
        sprite = _fit_centered(crop, (32, 32))
    ...
    if args.preview:
        ...
        pygame.image.save(_make_contact_sheet(previews), str(GENERATED_DIR / "missing_sprite_import_contact.png"))
    ...
    if skipped:
        ...
        for sprite_id in skipped:
            print(f"- {sprite_id}")
    if args.preview:
        print(f"Preview: {GENERATED_DIR / 'missing_sprite_import_contact.png'}")
    return 0
...
def _select_components(components: list[Component]) -> list[Component]:
    ...
    useful = sorted(useful, key=lambda component: (component.top + component.bottom) / 2)
    rows: list[list[Component]] = []
    for component in useful:
        center_y = (component.top + component.bottom) / 2
        for row in rows:
            row_center = sum((entry.top + entry.bottom) / 2 for entry in row) / len(row)
            if abs(center_y - row_center) < 70:
                row.append(component)
                break
        else:
            rows.append([component])
    rows = sorted(rows, key=lambda row: sum((entry.top + entry.bottom) / 2 for entry in row) / len(row))
    ordered: list[Component] = []
    for row in rows:
        ordered.extend(sorted(row, key=lambda component: (component.left + component.right) / 2))
    return ordered
