...
VISUAL_MAPPING: tuple[tuple[str, ...], ...] = (
    ...
    ("item_iron_boots",),
    ("item_antidote",),
    ("skip_green_potion_alt",),
    ("item_wooden_pickaxe",),
    ("item_fishing_rod",),
    ("skip_fishing_rod_alt",),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import the final leftover sprite atlas.")
    parser.add_argument("atlas", type=Path, help="Path to the transparent atlas PNG.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for component, sprite_ids in zip(components, VISUAL_MAPPING, strict=True):
        sprite = _fit_centered(atlas.subsurface(_padded_rect(component.rect(), atlas.get_size(), 16)).copy(), (32, 32))
    ...
    if args.preview:
        ...
        pygame.image.save(_make_contact_sheet(previews), str(GENERATED_DIR / "final_leftovers_import_contact.png"))
        pygame.image.save(_draw_detection_preview(atlas, components), str(GENERATED_DIR / "final_leftovers_detected.png"))
    ...
    if skipped:
        ...
        for sprite_id in skipped:
            print(f"- {sprite_id}")
    if args.preview:
        print(f"Preview: {GENERATED_DIR / 'final_leftovers_import_contact.png'}")
    return 0
...
def _select_components(components: list[Component]) -> list[Component]:
    useful = [
        ...
        if component.area >= 600 and component.width >= 20 and component.height >= 20
    ]
    ...
    for component in sorted(useful, key=lambda component: component.center_y):
        for row in rows:
            ...
            if abs(component.center_y - row_center) < 85:
